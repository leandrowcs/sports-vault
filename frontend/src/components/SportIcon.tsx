...
const competitionLogos: Record<FocusGroupId, string> = {
  nba: '/competitions/nba.png',
  nfl: '/competitions/nfl.png',
  futebol: '/competitions/brasileirao.png',
  selecao: '/competitions/brasil.png',
};

export function CompetitionIcon({ sport, size = 28 }: { sport: FocusGroupId; size?: number }) {
  const [failedSource, setFailedSource] = useState<string>();
  const source = competitionLogos[sport];
  if (failedSource === source) return <SportIcon sport={sport} size={size} />;
  return <img className="competition-logo" src={source} width={size} height={size} alt="" aria-hidden="true" decoding="async" onError={() => setFailedSource(source)} />;
}
...
export function NavigationIcon({ view, size = 20 }: { view: View; size?: number }) {
  ...
  return <CompetitionIcon sport={view} size={Math.max(size, 28)} />;
}
