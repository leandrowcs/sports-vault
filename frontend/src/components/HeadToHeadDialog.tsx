...
import { EVENT_TIME_ZONE } from "../helpers/eventDates";
        ...
        {filtered.length ? (
          <ul className="h2h-history">
            {filtered.map((event) => {
              ...
              return (
                <li key={event.id}>
                  <span>{new Intl.DateTimeFormat("pt-BR", { dateStyle: "medium", timeZone: EVENT_TIME_ZONE }).format(new Date(event.startsAt))}</span>
                  <b>{scoreA} - {scoreB}</b>
