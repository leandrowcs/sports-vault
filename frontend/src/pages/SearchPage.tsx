...
import { CompetitionIcon } from "../components/SportIcon";
  ...
  return (
    <section className="team-search" aria-label="Explorar times por esporte">
      ...
      <div className="competition-filters sport-search-filters" role="group" aria-label="Filtrar por esporte">
        ...
        {availableSports.map((sport) => <button key={sport.id} data-sport={getSportGroup(sport.id)} aria-pressed={sportId === sport.id} onClick={() => { setSportId(sport.id); setCompetitionId(null); setCollapsedIds(new Set()); }}><CompetitionIcon sport={getSportGroup(sport.id)} />{sport.name}<span>{uniqueTeams.filter((group) => group.sport === sport.id).length}</span></button>)}
      </div>
