...
export function VaultFeedHome({ events, getTeam, getLeague, leagues, onSelectEvent, onViewNba, onViewNfl }: {
  events: SportEvent[];
  ...
  function datedGames(games: SportEvent[]) {
    return groupSchedule(games, eventDay).map(([day, dayEvents]) => (
      <section className="schedule-day" key={day}>
        <h3>{day} <small>{eventTimeZoneLabel(new Date(dayEvents[0].startsAt))}</small></h3>
